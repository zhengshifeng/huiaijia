...
public class PageUtil {
	...
	public static String showPageStaticHtml(String url, int currentPage, int pages) {
		...
		if (pages > 0) {
			if (currentPage >= 1) {
				s = s + "<a href='" + url + "_1.html'>首页</a> ";
				if (currentPage > 1) {
					s = s + "<a href='" + url + "_" + (currentPage - 1) + ".html'>上一页</a> ";
				}
			}
			...
			if (beginPage <= pages) {
				...
				for (int j = 0; (i <= pages) && (j < 6); j++) {
					if (i == currentPage) {
						s = s + "<a class='this' href='" + url + "_" + i + ".html'>" + i + "</a> ";
					} else {
						s = s + "<a href='" + url + "_" + i + ".html'>" + i + "</a> ";
					}
					i++;
				}
				s = s + "　";
			...
			if (currentPage <= pages) {
				if (currentPage < pages) {
					s = s + "<a href='" + url + "_" + (currentPage + 1) + ".html'>下一页</a> ";
				}
				s = s + "<a href='" + url + "_" + pages + ".html'>末页</a> ";
			}
		}
		return s;
	}
