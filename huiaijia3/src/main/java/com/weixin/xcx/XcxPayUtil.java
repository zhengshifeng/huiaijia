...
public class XcxPayUtil {
	...
	private static final Logger log = Logger.getLogger(XcxPayUtil.class);
	...
	public static Map<String, Object> addWeixinOrder(HajRecharge recharge) {
		...
		map.put("appid", XcxConfigure.getAppid());
		map.put("mch_id", XcxConfigure.getMchid());
		...
		map.put("spbill_create_ip", XcxConfigure.getIP());
		map.put("notify_url", XcxConfigure.WX_PAY_NOTIFY_URL);
		...
		wxRequestBean.setAppid(XcxConfigure.getAppid());
		wxRequestBean.setMch_id(XcxConfigure.getMchid());
		...
		wxRequestBean.setNotify_url(XcxConfigure.WX_PAY_NOTIFY_URL);
		wxRequestBean.setOut_trade_no(out_trade_no);
		wxRequestBean.setSpbill_create_ip(XcxConfigure.getIP());
		...
		try {
			String returnStr = HttpClientUtil.sendPost(XcxConfigure.UNIFIED_ORDER_API, xmlData,
	...
	public static Map<String, Object> payQuery(String orderNum) {
		...
		try {
			String returnStr = HttpClientUtil.sendPost(XcxConfigure.PAY_QUERY_API, xmlData,
					HttpClientUtil.CONTENT_TYPE_XML, HttpClientUtil.UTF_8, HttpClientUtil.UTF_8);
	...
	private static String generateSign(HashMap<String, Object> map) {
		...
		sb.append("key=").append(XcxConfigure.getKey());
	...
	public static BigDecimal refundQuery(String orderNum) {
		...
		try {
			String returnStr = HttpClientUtil.sendPost(XcxConfigure.REFUND_QUERY_API, xmlData,
					HttpClientUtil.CONTENT_TYPE_XML, HttpClientUtil.UTF_8, HttpClientUtil.UTF_8);
	...
	public static boolean refund(String outTradeNo, String outRefundNo, int totalFee, int refundFee) throws Exception {
		...
		HttpPost httpPost = new HttpPost(XcxConfigure.REFUND_API);

		HashMap<String, Object> map = new HashMap<>();
		map.put("appid", XcxConfigure.getAppid());
		map.put("mch_id", XcxConfigure.getMchid());
		map.put("op_user_id", XcxConfigure.getMchid());
	...
	private static CloseableHttpClient initCert() throws Exception {
		...
		FileInputStream fileInputStream = new FileInputStream(new File(XcxConfigure.getCertLocalPath()));
		keyStore.load(fileInputStream, XcxConfigure.getCertPassword().toCharArray());

		// Trust own CA and all self-signed certs
		SSLContext sslcontext = SSLContexts.custom()
				.loadKeyMaterial(keyStore, XcxConfigure.getCertPassword().toCharArray())
				.build();
	...
	/**
	 * 生成微信小程序调起微信支付的paySign等数据
	 * @param prepay_id 微信支付预支付ID
	 */
	public static HashMap<String, Object> getPaySign(String prepay_id) {
		String timeStamp = String.valueOf(System.currentTimeMillis());
		String nonceStr = MD5Tools.MD5(timeStamp);
		String package_ = "prepay_id=" + prepay_id;

		StringBuilder paySignStr = new StringBuilder();
		paySignStr.append("appId=");
		paySignStr.append(XcxConfigure.getAppid());
		paySignStr.append("&nonceStr=");
		paySignStr.append(nonceStr);
		paySignStr.append("&package=");
		paySignStr.append(package_);
		paySignStr.append("&signType=MD5");
		paySignStr.append("&timeStamp=");
		paySignStr.append(timeStamp);
		paySignStr.append("&key=");
		paySignStr.append(XcxConfigure.getKey());

		log.info("paySign: " + paySignStr.toString());

		String paySign = MD5Tools.MD5(paySignStr.toString());
