...
<body style="background-color: #FFFFFF;">
	<div class="mainBox">
		<h3>商品实时分拣单</h3>
		<form action="getTodaySorting.html" method="post" name="userForm" id="userForm">
			<div class="filter">
				商品类别：
			...
			<div class="filter" style="height: 50px;">
				批量查找：
				<textarea class="textArea" rows="2" cols="70" placeholder="小区名称：景鹏大厦,景亿山庄" id="residentialList" name="residentialList"><c:forEach items='${sorting.residentialList }' var='residential' varStatus="vs">${residential }<c:if test="${!vs.last }">,</c:if></c:forEach></textarea>
		...
		<div class="search_div" style="line-height: 30px; text-align: right;">
			<span style="padding-right: 15px; font-size: 20px;">种类：${totalPurchase.commodityNumber}</span>
...
<%-- 			<span style="padding-right: 15px; font-size: 20px;">售出总价：${totalPurchase.sumSaleMoney}</span> --%>
		...
		<div class="page_and_btn">
			<div>
				...
				<a href="javascript:exportTodaySortOrder();" class="btn">
					<em>合并导出</em>
				</a>
	...
	<script type="text/javascript">
		...
		function selectBtypy() {
			...
			var url = "/commodityType/getBigType.html?commodityAttr="
					+ commodityAttr;
			$.get(url, function(data) {
				if (data.length > 0) {
					...
					$.each(data, function(i) {
						$("#bTypeName").append(
								"<option value='"+this.id+"'>" + this.typeName
										+ "</option>");
					});
				}
			}, "json");
		...
		function exportPurchase() {
			...
			document.location = "excel.html?beginTime=" + beginTime
					+ "&endTime=" + endTime + "&supplyChain=" + supplyChain
					+ "&commodityNo=" + commodityNo + "&bTypeName=" + bTypeName
					+ "&sTypeName=" + sTypeName;
		}

		function exportTodaySortOrder() {
			var beginTime = $("input[name='beginTime']").val();
			var endTime = $("input[name='endTime']").val();
			var commodityNo = $("input[name='commodityNo']").val();
			var bTypeName = $("#bTypeName").val();
			var sTypeName = $("#sTypeName").val();
			var residentialList = $("#residentialList").val();
			var commodityAttr = $("#commodityAttr").val();
			document.location = "exportTodaySortOrder.html?beginTime="
					+ beginTime + "&endTime=" + endTime + "&commodityNo="
					+ commodityNo + "&bTypeName=" + bTypeName + "&sTypeName="
					+ sTypeName + "&residentialList=" + residentialList+"&commodityAttr="+commodityAttr;
		}
		...
		function exportSalePurchase() {
			...
			var bTypeName = $("#bTypeName").val();
			var sTypeName = $("#sTypeName").val();
			document.location = "salexcel.html?beginTime=" + beginTime
					+ "&endTime=" + endTime + "&supplyChain=" + supplyChain
					+ "&commodityNo=" + commodityNo + "&bTypeName=" + bTypeName
					+ "&sTypeName=" + sTypeName + "";
		}
